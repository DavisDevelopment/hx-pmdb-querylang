...
class NewSqlParser {
	static var KWDS = [
		...
		"LEFT", "RIGHT", "INNER", "OUTER", "CROSS", "JOIN", "IN", "LIKE", "MOD", "DIV",
		"LIMIT", "OFFSET", "GROUP",
		"INTO", "VALUES", "IF", "ELSE", "CASE", "WHEN", "MATCH", "THEN", "FOR", "END"
	];
	static var OPKWDS = [
		"IN", "LIKE", "MOD", "DIV"
	];
	...
	*/
		...
		while (true) {
			...
			switch t {
				...
				case Ident(columnName):
					...
					res.push(new UpdateOp(type, symbol(columnName), e));
					if (maybe(Comma)) {
						continue;
					}
	...
	function parseExpr():Expr {
		...
		switch( t ) {
            case Kwd('NULL'), Ident(_.toUpperCase()=>'NULL'): 
                return parseExprNext(Expr.CNull);

            case Kwd('TRUE'), Ident(_.toUpperCase()=>'TRUE'): 
                return parseExprNext(Expr.CTrue);

            case Kwd('FALSE'), Ident(_.toUpperCase()=>'FALSE'):
				return parseExprNext(Expr.CFalse);
			...
			case BrOpen:
				...
				switch el {
					...
					case _:
						return parseExprNext(EArrayDecl(el));
				}
			...
			case Kwd('CASE'):
				...
				var r = Expr.ECase(branches, defaultBranch, expr);
				t = token();
				//Console.debug(t);
				push(t);
				//Console.debug(r);
	...
	function parseExpressionList(end: Token):Array<Expression> {
		return [for (e in parseExprList(end)) convertExprToExpression(e)];
	}
...
@:access(haxe.io)
@:access(ql.sql)
class SlaveInput extends Input {
