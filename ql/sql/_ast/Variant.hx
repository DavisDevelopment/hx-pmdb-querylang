...
abstract Variant(VariantType) from VariantType {
    ...
    /**
      builds a Variant from the given `Dynamic` value
      [TODO] add support for the full range of types representable
     **/
    ...
    /**
      returns `true` for `Int` and `Float` values, as well as for `String` values which represent a numeric value
      ```haxe
        containsOnlyDigits(10);//true
        containsOnlyDigits(12.3);//true
        containsOnlyDigits('10');//true
        containsOnlyDigits('$10');//false
        containsOnlyDigits(null);//false
      ```
     **/
	...
	public static function toDynamic(v:Variant):Dynamic {
		...
		if (v != null) {
			switch (v) {
				case VariantType.VT_Int(y):
					d = y;
				case VariantType.VT_Float(y):
					d = y;
				case VariantType.VT_String(y):
					d = y;
				case VariantType.VT_Bool(y):
					d = y;
				case VariantType.VT_Array(y):
					d = y;
				case VariantType.VT_Date(y):
					d = y;
			}
		}
		return d;
