...
import haxe.Int64;
import pm.Decimal;
import ql.sql.common.DateTimeInterval;
// import datetime.utils.DateTimeUtils;
import ql.sql.common.DateTimeUtils;
...
class DateTimeContainer {
	public var d(default, null): Num;

	public inline function new(n: Num) {
		this.d = n;
    }
	...
	public function toString():String {
        var self:DateTime = (this : DateTime);
		var Y = self.getYear();
		var M = self.getMonth();
		var D = self.getDay();
		var h = self.getHour();
		var m = self.getMinute();
		var s = self.getSecond();

		return '$Y-' + (M < 10 ? '0$M' : '$M') + '-' + (D < 10 ? '0$D' : '$D') + ' ' + (h < 10 ? '0$h' : '$h') + ':' + (m < 10 ? '0$m' : '$m') + ':' + (s < 10 ? '0$s' : '$s');
	}

    extern public inline function mod(n: Num):DateTimeContainer {
        this.d = n;
        return this;
    }
}

private typedef Num = #if pewp pm.Decimal #else Float #end;
...
@:allow(ql.sql.common)
@:forward(toString)
abstract DateTime (DateTimeContainer) from DateTimeContainer {
    /**
    * Constructor
    *
    ...
    */
    public inline function new(time: Float) {
        this = new DateTimeContainer(time + UNIX_EPOCH_DIFF);
    }

    public static inline function is(x: Dynamic):Bool {
        return (x is DateTimeContainer);
    }
     ...
     */
     static public inline function now () : DateTime {
         return new DateTime(
             #if cpp
             cast(untyped __global__.__hxcpp_date_now(), Float)
             #elseif js
             cast(untyped __js__("Math.floor(new Date().getTime() / 1000)"), Float)
             #elseif php
             cast(untyped __php__("time()"), Float)
             #elseif neko
             cast(untyped Date.date_now(), Float)
             #elseif java
                Math.ffloor(Int64.toInt(java.lang.System.currentTimeMillis() / 1000))
    ...
    */
    ...
    @:from extern static public inline function ofData(d: DateTimeContainer):DateTime {
        return (d : DateTime);
        // return d;
    }
    ...
    */
    ...
    @:from
    static public inline function fromDecimalTime(time: Decimal):DateTime {
        return new DateTime(time);
    }
    ...
    */
    ...
    public static function fromAny(value: Dynamic):DateTime {
        if ((value is DateTimeContainer)) return value;
        if ((value is Float)) return fromTime(cast(value, Float));
        if ((value is String)) return fromString(cast(value, String));
        if ((value is Date)) return fromDate(cast(value, Date));
        if (TypedValue.is(value)) return fromAny(cast(value, ql.sql.common.TypedValue.CTypedValue).value);
        if ((value is pm.math.bigint.DecimalImpl)) return fromDecimalTime(cast(value, pm.math.bigint.DecimalImpl));
        throw new pm.Error('Cannot cast ${Type.typeof(value)} to DateTime');
    }
    ...
    */
    public function getYear():Int {
        var cquads    = Std.int(this.d / SECONDS_IN_CQUAD) * SECONDS_IN_CQUAD;
        var centuries = Std.int((this.d - cquads) / SECONDS_IN_CENTURY) * SECONDS_IN_CENTURY;
        if (centuries > 3 * SECONDS_IN_CENTURY) {
            centuries -= SECONDS_IN_CENTURY;
        }
        var quads = Std.int((this.d - cquads - centuries) / SECONDS_IN_QUAD) * SECONDS_IN_QUAD;
        var years : Int = Std.int((this.d - cquads - centuries - quads) / SECONDS_IN_YEAR);
    ...
    /**
    * Get unix timestamp of a first second of this.d year
    *
    */
    ...
    */
    public inline function getHour () : Int {
        return Std.int((this.d - Math.ffloor(this.d / SECONDS_IN_DAY) * SECONDS_IN_DAY) / SECONDS_IN_HOUR);
    }//function getHour()
    ...
    */
    public inline function getMinute () : Int {
        return Std.int((this.d - Math.ffloor(this.d / SECONDS_IN_HOUR) * SECONDS_IN_HOUR) / SECONDS_IN_MINUTE);
    }//function getMinute()
    ...
    */
    public inline function getSecond () : Int {
        return Std.int(this.d - Math.ffloor(this.d / SECONDS_IN_MINUTE) * SECONDS_IN_MINUTE);
    }//function getSecond()
    ...
    */
    ...
    public inline function getTime () : Float {
        var tmp = (this.d - UNIX_EPOCH_DIFF);//.toFloat();
        return tmp;
    }


    /**
    * Create standart `Date` class instance using unix timestamp of this.d one
    *
    */
    @:to public inline function getDate():Date {return Date.fromTime(getTime() * 1000);}
    ...
    *
    */
    @:op(A + B)  private static inline function mathPlus1(d:DateTime, period:DTPeriod):DateTime return d.add(period);
    @:op(A + B)  private static inline function mathPlus2 (period:DTPeriod, d:DateTime):DateTime return d.add(period);
    @:op(A += B) 
    private inline function mathPlus3(period:DTPeriod):DateTime {
        return this.mod(add(period).getTime() + UNIX_EPOCH_DIFF);
    }
    @:op(A - B)  private inline function mathMinus1 (period:DTPeriod) : DateTime return sub(period);
    @:op(A += B) 
    private inline function mathMinus2(period: DTPeriod):DateTime {
        // return this.d = sub(period).getTime() + UNIX_EPOCH_DIFF;
        return this.mod(add(period).getTime() + UNIX_EPOCH_DIFF);
    }
    ...
    */
    @:op(A - B)  
    private static inline function dtiCreate(begin:DateTime, end:DateTime):DateTimeInterval {
        return DateTimeInterval.create(begin, end);
    }
    @:op(A - B)  private static inline function dtiMinus(d:DateTime, dti:DateTimeInterval):DateTime return dti.subFrom(d.getTime());
    ...
    @:op(A -= B) private inline function dtiMinus2 (dti:DateTimeInterval) : DateTime return this.d = dti.subFrom(getTime()).getTime() + UNIX_EPOCH_DIFF;
    @:op(A += B) private inline function dtiPlus3 (dti:DateTimeInterval) : DateTime return this.d = dti.addTo(getTime()).getTime() + UNIX_EPOCH_DIFF;
