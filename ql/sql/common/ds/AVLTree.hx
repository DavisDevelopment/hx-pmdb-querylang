...
class AVLTree<Key, Value> {
   ...
   private var mSize(default, null):Int;
   private final mCompareKeys: (Key, Key)->Int;
   
	public function new(compare:Key->Key->Int, ?options:{?unique:Bool}):Void {
		root = null;
		mSize = 0;
		unique = options.unique != null ? options.unique : false;

		mCompareKeys = compare;
	...
	**/
	// extern inline function checkValueEquality(a:Value, b:Value):Bool {
   //    // return model.checkValueEquality(a, b);
   //    return pm.Helpers.strictEq(a, b);
   // }
   
   public function lessThan(max:Key, inclusive:Bool=false):Array<Value> {
		// TODO
		throw new pm.Error.NotImplementedError();
   }

   public function greaterThan(min:Key, inclusive:Bool=false):Array<Value> {
		// TODO
		throw new pm.Error.NotImplementedError();
   }

   public function between(min:Key, max:Key, ?inclusive:Bool, ?minInclusive:Bool, ?maxInclusive:Bool):Array<Value> {
      switch [inclusive, minInclusive, maxInclusive] {
         case [null, null, null]:
            inclusive = minInclusive = maxInclusive = false;
         case [_, null, null]:
            minInclusive = maxInclusive = inclusive;
         case [min, max, null]:
            minInclusive = min;
            maxInclusive = max;
         default:
	...
	**/
	function _betweenBounds_(query:{?lower:BoundingValue<Key>, ?upper:BoundingValue<Key>}, ?lbm:Key->Bool, ?ubm:Key->Bool, root:AVLTreeNode<Key, Value>,
			?res:Array<Value>):Array<Value> {
		res = res != null ? res : [];
	...
	**/
   @:pure 
   public function getNode(key: Key):Null<AVLTreeNode<Key, Value>> {
		return root == null ? null : _get_nonrecursive(key, root, compareKeys);
	}
	...
	**/
   @:pure 
   final public function get(key: Key):Null<Array<Value>> {
      final node = inline getNode(key);
	...
	**/
	private function _get(key:Key, root:Null<Node<Key, Value>>):Null<Node<Key, Value>> {
      var cmp = this.compareKeys(key, root.key);
      if (same(0, cmp))
         return root;
   ...
   inline static function _get_nonrecursive<Key, Item>(key:Key, root:Node<Key, Item>, compare:Key->Key->Int):Node<Key, Item> {
      var node = root, cmp:Int;
      var q = [node];
      while (q.length != 0) {
         node = q.shift();
         ...
         if (cmp == 0)
            ...
            if (cmp < 0 && node.left != null) q.unshift(node.left);
            if (cmp < 0 && node.right != null) q.unshift(node.right);
         }
      }
      return null;
   }
